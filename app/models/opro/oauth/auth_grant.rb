class Opro::Oauth::AuthGrant < ActiveRecord::Base
  ...
  def self.find_or_create_by_user_app(user, app, token, type)
    app_id = app.is_a?(Integer) ? app : app.id
    auth_grant  =   self.where(:user_id  => user.id, :application_id => app_id, :devise_token => token).first
    auth_grant  ||= begin
      auth_grant                = self.new
      auth_grant.user_id        = user.id
      auth_grant.application_id = app_id
      auth_grant.devise_token   = token
      auth_grant.devise_type    = type
      auth_grant.save
      auth_grant
    end
  end
